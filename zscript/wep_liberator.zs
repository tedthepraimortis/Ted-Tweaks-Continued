...
class LiberatorRifle:AutoReloadingThingy{
	default{
		...
		hdweapon.loadoutcodes "
			...
			\cuirons - 0/1, whether to use iron sights or not
	...
	override string,double getpickupsprite(bool usespare){
		...
		return ((wep0&LIBF_NOBULLPUP)?"BRLL":
		(wep0&LIBS_IRONS)?"IBFL":
		(wep0&LIBS_IRONS&LIBF_NOBULLPUP)?"IBLL":
		"BRFL")
		..spr.."0",1.;
	}
	override void DrawHUDStuff(HDStatusBar sb,HDWeapon hdw,HDPlayerPawn hpl){
	...
	override void DrawSightPicture(
		...
		if(hdw.weaponstatus[0]&LIBF_GRENADEMODE)sb.drawgrenadeladder(hdw.airburst,bob);
			...
			if(hdw.weaponstatus[0]&LIBS_IRONS){
				int cx,cy,cw,ch;
...
firegrenade:
	...
	override void loadoutconfigure(string input){
		...
		int irons=getloadoutvar(input,"irons",1);
		if(!irons)weaponstatus[0]&=~LIBS_IRONS;
...
class LiberatorRandom:IdleDummy{
	...
	spawn:
		TNT1 A 0 nodelay{
			...
			if(!random(0,5))lll.weaponstatus[0]|=LIBS_IRONS;
