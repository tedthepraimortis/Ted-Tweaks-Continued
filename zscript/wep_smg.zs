...
class HDSMG:HDWeapon{
	...
	override string,double getpickupsprite(bool usespare){
		...
		return ((wep0&SMGF_REFLEXSIGHT)?"SMSN":(wep0&SMGF_NOLAUNCHER)?"SMGN":"SMLN")
	...
	firefrag:
		#### B 2{
			if(!(invoker.weaponstatus[SMGS_FLAGS]&SMGF_GRENADELOADED)){
				setweaponstate("nope");
				return;
			}
			A_FireHDGL();
			invoker.weaponstatus[SMGS_FLAGS]&=~SMGF_GRENADELOADED;
	...
	altfire:
		#### A 1 offset(0,34){
			if(invoker.weaponstatus[0]&SMGF_NOLAUNCHER)return;
			invoker.weaponstatus[0]^=SMGF_GLMODE;
			invoker.airburst=0;
			A_SetCrosshair(21);
			A_SetHelpText();
		}goto nope;
	...
	altreload:
		#### A 0{
			invoker.weaponstatus[SMGS_FLAGS]&=~SMGF_UNLOADONLY;
			if(
				!(invoker.weaponstatus[SMGS_FLAGS]&SMGF_NOLAUNCHER)
				&&!(invoker.weaponstatus[SMGS_FLAGS]&SMGF_GRENADELOADED)
	...
	unloadgrenade:
		...
		#### B 2 offset(8,48){
			...
			A_MuzzleClimb(-0.3,-0.3);
			if(invoker.weaponstatus[SMGS_FLAGS]&SMGF_GRENADELOADED)A_StartSound("weapons/grenreload",CHAN_WEAPON);
		}
		#### B 10 offset(10,49){
			if(!(invoker.weaponstatus[SMGS_FLAGS]&SMGF_GRENADELOADED)){
				if(!(invoker.weaponstatus[SMGS_FLAGS]&SMGF_UNLOADONLY))A_SetTics(3);
				return;
			}
			invoker.weaponstatus[SMGS_FLAGS]&=~SMGF_GRENADELOADED;
			if(
		...
		#### B 0 A_JumpIf(invoker.weaponstatus[SMGS_FLAGS]&SMGF_UNLOADONLY,"greloadend");
	loadgrenade:
		#### B 4 offset(10,50) A_StartSound("weapons/pocket",CHAN_WEAPON,CHANF_OVERLAP);
		#### BBB 8 offset(10,50) A_MuzzleClimb(frandom(-0.2,0.8),frandom(-0.2,0.4));
		#### B 18 offset(8,50){
	...
	greloadend:
		#### B 4 offset(4,44) A_StartSound("weapons/grenopen",CHAN_WEAPON);
		#### B 1 offset(0,40);
	...
	override void loadoutconfigure(string input){
		...
		if(!nogl){
			weaponstatus[0]&=~SMGF_NOLAUNCHER;
		}else if(nogl>0){
			weaponstatus[0]|=SMGF_NOLAUNCHER;
			weaponstatus[0]&=~SMGF_GRENADELOADED;
		}
		if(!(weaponstatus[0]&SMGF_NOLAUNCHER)){
			weaponstatus[0]|=SMGF_GRENADELOADED;
			hdammotype2=null;
		}
	}
